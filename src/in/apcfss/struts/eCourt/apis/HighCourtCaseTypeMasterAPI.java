...
public class HighCourtCaseTypeMasterAPI {
	...
	public static void main(String[] args) throws Exception {
		...
		try {
			...
			String opVal = ECourtAPIs.getSelectParam(20);
			
			
			 sql="select distinct est_code from ecourts_mst_bench";
			...
			while(rs.next())
			{
				inputStr =  "est_code="+rs.getString("est_code");//ECourtAPIs.getInputStringValue(opVal);
				...
				if (resp != null && !resp.equals("")) {
					try {
						processCaseTypesMasterResponse(resp, opVal, con, rs.getString("est_code"));
					} catch (Exception e) {
						e.printStackTrace();
					}
				}
	...
	public static void processCaseTypesMasterResponse(String resp, String fileName, Connection con, String est_code)
			throws Exception {
		String response_str = "";
		...
		if ((resp != null) && (!resp.equals("")) && (!resp.contains("INVALID_TOKEN"))) {
			...
			if ((jObjCaseData != null)) {
				...
				for (int i = 1; i <= jObjCaseData.length(); i++) {//jObjActsData.length()
					if(jObjCaseData.has("casetype" + i))
					{
						jObjActsInnerData = new JSONObject(jObjCaseData.get("casetype" + i).toString());
	
						sql = "INSERT INTO apolcms.ecourts_mst_casetypes(case_type_id ,est_code , case_type_code , case_type_name) VALUES(" + i + ",'"
								+ est_code + "',  '"
								+ ImportECourtsData.checkStringJSONObj(jObjActsInnerData, "case_type") + "',  '"
								+ ImportECourtsData.checkStringJSONObj(jObjActsInnerData, "type_name") + "')";
			...
			System.out.println("Successfully saved. casetype..executedSqls:" + executedSqls);
