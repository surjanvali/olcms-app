...
public class SessionChecking implements Filter {
	...
	public void doFilter(ServletRequest servletrequest, ServletResponse servletresponse, FilterChain filterChain)
		...
		if (actionFile != null && (actionFile.equalsIgnoreCase("Login") || actionFile.equalsIgnoreCase("Logout")
				|| actionFile.equalsIgnoreCase("AdvocateRegistation") || actionFile.equalsIgnoreCase("AjaxModels"))) {
		...
		if ((session != null) && (session.getAttribute("userid") != null)) {
			if (sid != null && !sid.equals("") && !sid.equals("null")) {
				 ...
				 * session.setAttribute("sesAcvtiveLinks", ids); else
				 */
		 ...
		 * ((HttpServletRequest)servletrequest).getRequestDispatcher("sessionExpired.do"
		 * ); rd.forward(servletrequest, servletresponse); }
		 */
	...
	public void getBreadCrumb(List usrServices, String serviceId) {
		...
		for (Object obj : usrServices) {
			...
			// System.out.println("ourt ..." +m.get("serviceid").toString() +" pass ::
			// "+serviceId);
		...
		if (!parentId.equals("0")) {
			...
			for (Object obj : usrServices) {
				...
				if (m.get("service_id").toString().equals(parentId)) {
					...
					getServiceId(usrServices, m.get("service_id").toString(), m.get("parent_id").toString(),
