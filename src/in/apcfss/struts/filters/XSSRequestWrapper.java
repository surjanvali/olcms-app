...
public final class XSSRequestWrapper extends HttpServletRequestWrapper implements HttpServletRequest {
    ...
    private static Pattern[] patterns = new Pattern[]{
        ...
        Pattern.compile(">", Pattern.CASE_INSENSITIVE | Pattern.MULTILINE | Pattern.DOTALL),
        Pattern.compile("eval\\((.*)\\)", Pattern.CASE_INSENSITIVE | Pattern.MULTILINE | Pattern.DOTALL),
        Pattern.compile("[\\\"\\\'][\\s]*((?i)javascript):(.*)[\\\"\\\']", Pattern.CASE_INSENSITIVE | Pattern.MULTILINE | Pattern.DOTALL),
    ...
    {  
        ...
        for (int i = 0; i < count; i++) 
        {
        	...
        	// Forwarding each request form value to stripXSS() method sent by the user for filtering unintended content
        	// After  filtering saving all the requested form values in the encodedValues array
    ...
    private String stripXSS(String value) 
    {
        if (value != null) 
        {
        	...
        	try {
	            ...
	            {	            	
	                value = scriptPattern.matcher(value).replaceAll(""); // If unintended script id found, replacing it with the space or empty string	                
	            }
        	}
