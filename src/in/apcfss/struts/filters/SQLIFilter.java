...
public class SQLIFilter implements Filter {
	...
	public void doFilter(ServletRequest request, ServletResponse response, FilterChain chain)
    	...
    	String[] AttackPattern={"1'","1' "," 1'","1' OR","1' OR '1'=1","1'%20","1 '%20","'%20","'%20OR","' OR","'%20OR%20'1","'%20'1","1 '%20OR%20'1'%3d'1","'%20OR%20'1'%3d'1"," '%20",
    							//" CREATE"," ALTER"," DROP"," DROP TABLE"," RENAME"," SELECT"," SELECT *"," SELECT * FROM"," INSERT INTO"," UPDATE"," DELETE FROM"," REVOKE"," @@VERSION",
    			" CREATE"," ALTER"," DROP"," DROP TABLE"," RENAME"," INSERT INTO"," UPDATE"," DELETE FROM"," REVOKE"," @@VERSION",
    	...
    	// VERIFY THE ENTIRE REQUESTED DATA SENT BY THE USER, WHETHER EXIST IN THE ATTACK PATTERN OR NOT 
    	String qry = req1.getQueryString(); //Query String
    	
    	System.out.println("getQueryString : "+req1.getQueryString());
    	System.out.println("getRequestURL : "+req1.getRequestURL());
		...
		for(i=0;i<AttackPattern.length;i++)
		{
			...
			{				
				...
				fnd=(qry.toLowerCase()).indexOf(AttackPattern [i].toLowerCase());	// Total Query String checking with the individual attack patterns declared above			
		...
		if (chk==true) // IF ATTACK NOT FOUND IN  the Query String and checking the data entered by user (Form properties values)
		{
			...
			for (; enumm.hasMoreElements(); ) 
			{
	           ...
	           System.out.println("name : "+name+"  value : "+value);
	            ...
	            for(i=0;i<AttackPattern.length;i++)
	    		{
        			...
        			if(value.equalsIgnoreCase(AttackPattern [i].toLowerCase().trim()))  // Attack pattern found in the request paramter value
					{
						chk=false; //Attack Pattern Present,  SET CHK VARIABLE FALSE
						
						// Print the Form property Name, Parameter Value and the corresponding Attacking Pattern matched  
						System.out.println("Attack Pattern Present (SQL Injection @ request parameter): Param name="+name+", Param value="+value+", Pattern value="+AttackPattern[i]+", Index="+i);
		            ...
		            if (chk==false) //Verify the CHK variable. if chk==false (attack patter found) and if chk==true (attack patter not found) in this inner for loop
		    		{	    				
		    			break;	    		
		    		}
	    		}
	            if (chk==false)  //Verify the CHK variable. if chk==false (attack patter found) and if chk==true (attack patter not found) in the enum for loop
	    		{	    				
	    			break;	    		
	    		}
			} // for loop enumm
		...
		{			
			...
			// Set the session expire page if the any insecure request is made to application 
