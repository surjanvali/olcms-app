...
public class ScannedDocumentrReportAction extends DispatchAction {
	@Override
	public ActionForward unspecified(ActionMapping mapping, ActionForm form, HttpServletRequest request,
		...
		try {
			...
			} else // if(roleId.equals("3") || roleId.equals("4"))
			{
				...
				sql=" select a1.reporting_dept_code as deptcode,dn1.description,sum(total_cases) as  total_cases "
						...
						+ " count(*) as total_cases from ecourts_case_data a  "
						+ " inner join dept_new dn on (a.dept_code=dn.dept_code)  "
						+ " where length(scanned_document_path) >10 group by  reporting_dept_code,a.dept_code ) a1 "
						+ " inner join dept_new dn1 on (a1.reporting_dept_code=dn1.dept_code)  group by a1.reporting_dept_code,dn1.description order by 1";

				request.setAttribute("HEADING", "Sect. Dept. Wise Scanned Document Report");
	...
	public ActionForward HODwisedetails(ActionMapping mapping, ActionForm form, HttpServletRequest request,
		...
		try {
			...
			sql = "select a.dept_code as deptcode , upper(d.description) as description,count(*) as total_cases  "
					+ "from ecourts_case_data a " + "inner join dept_new d on (a.dept_code=d.dept_code) "
					+ "where d.display = true and  length(scanned_document_path) >10  and (d.reporting_dept_code='" + deptId + "' or a.dept_code='" + deptId
					+ "') " + sqlCondition + "group by a.dept_code , d.description order by 1";

			request.setAttribute("HEADING", "HOD Wise Scanned Document Abstract Report for " + deptName);
	...
	public ActionForward getCasesList(ActionMapping mapping, ActionForm form, HttpServletRequest request,
		...
		try {
			...
			heading = "Scanned Document Cases List for " + deptName;
			...
			sql = "select a.*, "
					...
					+ " and  POSITION('RECORD_NOT_FOUND' in order_document_path) = 0"
					+ " and POSITION('INVALID_TOKEN' in order_document_path) = 0 ) order by cino, order_date desc) c group by cino ) b"
					+ " on (a.cino=b.cino) inner join dept_new d on (a.dept_code=d.dept_code) "+condition+" where d.display = true and  length(scanned_document_path) >10 ";
