	...
	public ActionForward getSubmitCategory(ActionMapping mapping, ActionForm form, HttpServletRequest request,
		...
		try {
			...
			if (data != null && !data.isEmpty() && data.size() > 0) {
				...
				sql = "update ecourts_case_category_wise_data set finance_category='"+cform.getDynaForm("fin_category").toString()+"', " +
						  " work_name='"+cform.getDynaForm("nameOfwork").toString()+"',est_cost='"+cform.getDynaForm("estCost").toString()+"'   ," +
						  " admin_sanction='"+cform.getDynaForm("adminSanction").toString()+"',grant_val='"+cform.getDynaForm("grant").toString()+"'," +
						  " cfms_bill='"+cform.getDynaForm("cfmsBill").toString()+"',bill_status='"+cform.getDynaForm("status").toString()+"'," +
						  " bill_amount='"+cform.getDynaForm("billAmount").toString()+"',efile_com_no='"+cform.getDynaForm("fileComNo").toString()+"'," +
						  " bill_remarks='"+cform.getDynaForm("remarks").toString()+"'  where cino='" +cInoo+ "' ";
