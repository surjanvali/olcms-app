...
public class RegisterSubNodalOfficerAction extends DispatchAction {
	@Override
		...
		try {
			...
			if (userId == null || roleId == null || userId.equals("") || roleId.equals("")) {
				...
				if(roleId.trim().equals("2"))
				{
					...
					sql="select sdeptcode||deptcode,sdeptcode||deptcode||'-'||upper(trim(description)) as description from dept where sdeptcode='" + userId.substring(0,3) + "' and deptcode!='01' order by sdeptcode,deptcode";
					sql="select sdeptcode||deptcode,sdeptcode||deptcode||'-'||upper(trim(description)) as description  from dept a"
							+ "	inner join (select substr(trim(global_org_name),1,5) as global_org_name,count(*) from nic_data group by substr(trim(global_org_name),1,5)) b on (a.sdeptcode||a.deptcode=b.global_org_name)"
							+ " where sdeptcode='" + userId.substring(0,3) + "' and deptcode!='01' order by sdeptcode ";
				...
				if(roleId.trim().equals("2"))
				{
					sql = "select slno, user_id, designation, employeeid, mobileno, emailid, aadharno, b.fullname_en, designation_name_en,upper(trim(d.description)) as description " +
		...
		try {
			...
			if (userId == null || roleId == null || userId.equals("") || roleId.equals("")) {
				...
				if (DatabasePlugin.NonZeroValidation(designationId) == true
					...
					sql = "insert into nodal_officer_details (dept_id, user_id, designation, employeeid, mobileno, emailid, aadharno, inserted_by, inserted_ip, inserted_time) " +
							"values (?,?,?,?,?,?,?,?,?,now())";
					...
					if (status == 1){
						...
						if(tableName.equals("nic_data")) {
							...
							sql = "insert into users (userid, password, user_description, created_by, created_on, created_ip, dept_id, dept_code, user_type) select a.emailid, md5('olcms@2021'), b.fullname_en, '"
									...
									+ " inner join dept d on (d.sdeptcode||d.deptcode='"
							...
							sql = "insert into users (userid, password, user_description, created_by, created_on, created_ip, dept_id, dept_code, dist_id, user_type) select a.emailid, md5('olcms@2021'), b.fullname_en, '"
									+ userId + "', now(),'" + request.getRemoteAddr()
									+ "',d.dept_id,d.sdeptcode||d.deptcode as deptcode,"+CommonModels.checkStringObject(session.getAttribute("dist_id"))+", 10 from nodal_officer_details a inner join (select distinct employee_id,fullname_en from "
									+ tableName + ") b on (a.employeeid=b.employee_id) " + ""
									+ " inner join dept d on (d.sdeptcode||d.deptcode='"+ (String) cform.getDynaForm("deptId") + "')" 
