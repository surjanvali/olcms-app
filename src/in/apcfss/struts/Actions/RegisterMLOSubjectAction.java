...
public class RegisterMLOSubjectAction extends DispatchAction {
	public ActionForward unspecified(ActionMapping mapping, ActionForm form,
		...
		try {
			...
			//and designation_id not in (select designation from mlo_subject_details where user_id='"+deptCode.substring(0,5)+"')
			...
			sql = "select slno, user_id, designation, employeeid, mobileno, emailid, aadharno, b.fullname_en, designation_name_en, a.subject_desc from mlo_subject_details a "
		...
		try {
			...
			if (!(session.getAttribute("role_id").toString().trim().equals("3") || session.getAttribute("role_id").toString().trim().equals("2"))) {
				...
				if (DatabasePlugin.NonZeroValidation(designationId) == true
					...
					sql = "insert into mlo_subject_details (user_id, designation, employeeid, mobileno, emailid, aadharno, inserted_by, inserted_ip, inserted_time, subject_desc) values (?,?,?,?,  ?,?,?,?, now(),?)";
					...
					ps.setString(++column, CommonModels.checkStringObject(cform.getDynaForm("subjectDesc")));
					...
					if (status == 1){
						...
						sql="insert into users (userid, password, user_description, created_by, created_on, created_ip, dept_id , dept_code, user_type) "
								...
								+ "',15  from mlo_subject_details a inner join (select distinct employee_id,designation_id,designation_name_en,fullname_en from nic_data) b on (a.employeeid=b.employee_id and a.designation=b.designation_id) "
						...
						sql="insert into user_roles (userid, role_id) values ('"+emailId+"','15')";
			...
			cform.setDynaForm("subjectDesc", null);
			DatabasePlugin.close(con, ps, null);
		}
	...
	public ActionForward sendCredentialsSMS(ActionMapping mapping,
		...
		try {
			...
			if (emailId != null && !emailId.equals("") && userType != null && !userType.equals("")) {
				...
				else if(userType.equals("MLO-SUB")) {
					sql = "select mobileno from mlo_subject_details where emailid='"+emailId+"'";
				}
